...
console.log(\`Protocolo completado: \${contador} intentos\`)`,
  codeHints: [
    'class ErrorFabrica extends Error {',
    '  constructor(message: string) { super(message); this.name = "ErrorFabrica" }',
    '}',
    'class ErrorEnergia extends ErrorFabrica {',
    '  constructor(public nivelActual: number) {',
    '    super(`Energía insuficiente: ${nivelActual}% (mínimo 20%)`)',
    ...
    'for (const [energia, pieza] of intentos) {',
    '  contador++',
    '  try { console.log(ensamblarConValidacion(energia, pieza)) }',
    '  catch (e) {',
    '    if (e instanceof ErrorEnergia) console.log(`[ErrorEnergia] ${e.message}`)',
    '    else if (e instanceof ErrorPieza) console.log(`[ErrorPieza] ${e.message}`)',
    '  }',
    '}',
    'console.log(`Protocolo completado: ${contador} intentos`)',
  ],

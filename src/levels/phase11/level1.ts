...
console.log('Sistema operativo ✅')`,
  codeHints: [
    ...
    '  agregar(item: T): Result<T, string> {',
    '    if (this.items.has(item.id)) return err(`ID duplicado: ${item.id}`)',
    '    this.items.set(item.id, item); return ok(item)',
    '  }',
    '  obtener(id: string): Result<T, string> {',
    '    const item = this.items.get(id)',
    '    if (!item) return err(`No encontrado: ${id}`); return ok(item)',
    '  }',
    '  actualizarStock(id: string, delta: number): Result<number, string> {',
    '    const res = this.obtener(id)',
    '    if (!res.ok) return err(res.error)',
    '    const nuevoStock = res.value.stock + delta',
    '    if (nuevoStock < 0) return err(`Stock insuficiente`)',
    '    this.items.set(id, { ...res.value, stock: nuevoStock }); return ok(nuevoStock)',
    '  }',
    '  resumen(): string { ... }',
    '}',
    '',
    'async function procesarPedido(almacen, id, cantidad): Promise<Result<string, string>> {',
    '  await new Promise(r => setTimeout(r, 30))',
    '  const res = almacen.actualizarStock(id, -cantidad)',
    '  if (!res.ok) return err(`Pedido fallido: ${res.error}`)',
    '  return ok(`Pedido confirmado: ${cantidad}x ${id}`)',
    '}',
    '',
    'const almacen = new Almacen<Producto>()',
    'almacen.agregar({ id: "P001", nombre: "Perno M6", precio: 0.5, stock: 100 })',
    'const resultados = await Promise.all([procesarPedido(almacen, "P001", 30), ...])',
    'resultados.forEach(r => { if (r.ok) console.log(`✅ ${r.value}`) })',
    'console.log("Sistema operativo ✅")',
  ],
